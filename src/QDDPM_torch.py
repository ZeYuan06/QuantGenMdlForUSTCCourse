...
class DiffusionModel(nn.Module):
    ...
    def HaarSampleGeneration(self, Ndata, seed):
        ...
        return torch.from_numpy(states_T).cfloat().to(self.device)
    ...
    def set_diffusionData_t(self, t, inputs, diff_hs, seed):
        ...
        diff_hs_cpu = diff_hs.to(cpu_device)
        inputs_cpu = inputs.to(cpu_device)
        np.random.seed(seed)
        phis = (
            torch.rand(self.Ndata, 3 * self.n * t, device=cpu_device) * np.pi / 4.0
            - np.pi / 8.0
        )
        phis = phis * (diff_hs_cpu.repeat(3 * self.n))
        if self.n > 1:
            # set homogenous RZZ gate angles
            gs = torch.rand(self.Ndata, t, device=cpu_device) * 0.2 + 0.4
            gs *= diff_hs_cpu
        states = torch.zeros((self.Ndata, 2**self.n), device=cpu_device).cfloat()
        for i in range(self.Ndata):
            if self.n > 1:
                states[i] = self.scrambleCircuit_t(t, inputs_cpu[i], phis[i], gs[i])
            else:
                states[i] = self.scrambleCircuit_t(t, inputs_cpu[i], phis[i])
        return states.to(target_device)
...
class QDDPM(nn.Module):
    def __init__(self, n, na, T, L, device="cpu"):
        ...
        self.backCircuit_vmap = K.vmap(
            partial(backCircuit, n_tot=self.n_tot, L=L), vectorized_argnums=0
        )

    def set_diffusionSet(self, states_diff):
        self.states_diff = torch.from_numpy(states_diff).cfloat().to(self.device)
    ...
    def randomMeasure(self, inputs):
        ...
        """
        m_probs = (
            torch.abs(inputs.reshape(inputs.shape[0], 2**self.na, 2**self.n)) ** 2
        ).sum(dim=2)
        m_res = torch.multinomial(
            m_probs, num_samples=1
        ).squeeze()  # measurment results
        indices = 2**self.n * m_res.view(-1, 1) + torch.arange(
            2**self.n, device=self.device
        )
        post_state = torch.gather(inputs, 1, indices)
        norms = torch.sqrt(torch.sum(torch.abs(post_state) ** 2, axis=1)).unsqueeze(
            dim=1
        )
    ...
    def prepareInput_t(self, inputs_T, params_tot, t, Ndata):
        ...
        self.input_tplus1[:, : 2**self.n] = inputs_T.to(self.device)
        params_tot = torch.from_numpy(params_tot).float().to(self.device)
        with torch.no_grad():
            for tt in range(self.T - 1, t, -1):
                self.input_tplus1[:, : 2**self.n] = self.backwardOutput_t(
                    self.input_tplus1, params_tot[tt]
                )
    ...
    def backDataGeneration(self, inputs_T, params_tot, Ndata):
        ...
        """
        states = torch.zeros((self.T + 1, Ndata, 2**self.n_tot), device=self.device).cfloat()
        states[-1, :, : 2**self.n] = inputs_T.to(self.device)
        params_tot = torch.from_numpy(params_tot).float().to(self.device)
        with torch.no_grad():
            for tt in range(self.T - 1, -1, -1):
                states[tt, :, : 2**self.n] = self.backwardOutput_t(
                    states[tt + 1], params_tot[tt]
                )
        return states
...
def naturalDistance(Set1, Set2):
    ...
    r11 = 1.0 - torch.mean(torch.abs(contract("mi,ni->mn", Set1.conj(), Set1)) ** 2)
    r22 = 1.0 - torch.mean(torch.abs(contract("mi,ni->mn", Set2.conj(), Set2)) ** 2)
    r12 = 1.0 - torch.mean(torch.abs(contract("mi,ni->mn", Set1.conj(), Set2)) ** 2)
...
def WassDistance(Set1, Set2):
    ...
    """
    D = 1.0 - torch.abs(Set1.conj() @ Set2.T) ** 2.0
    emt = torch.empty(0)
